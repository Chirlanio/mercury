...
class CpAdmsPesqUsuario {
    ...
    private function pesqUsuarioComp() {
        ...
        $paginacao->paginacao("SELECT COUNT(user.id) AS num_result FROM adms_usuarios user WHERE nivac.ordem >=:ordem AND user.nome LIKE '%' :nome '%' AND user.email LIKE '%' :email '%' ", "ordem=" . $_SESSION['ordem_nivac'] . "&nome={$this->Dados['nome']}&email={$this->Dados['email']}");
