...
class CpAdmsPesqUsuarioTreinamento {
    ...
    private function pesqUsuarioNome() {
        ...
        $paginacao->paginacao("SELECT COUNT(user.id) AS num_result FROM adms_users_treinamentos user INNER JOIN adms_niveis_acessos nivac ON nivac.id=user.adms_niveis_acesso_id WHERE nivac.ordem >=:ordem AND user.nome LIKE '%' :nome '%' ", "ordem=" . $_SESSION['ordem_nivac'] . "&nome={$this->Dados['nome']}");
