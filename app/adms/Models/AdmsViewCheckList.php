...
class AdmsViewCheckList {
    ...
    public function viewCheckList(string $DadosId) {
        ...
        $viewCheckList->fullRead("SELECT cls.id cls_id, cls.hash_id, cls.adms_store_id, lj.nome stores,
                cls.adms_check_list_area_id, cla.id cla_id, cla.name name_area, cla.weigth, cla.order_area,
                cls.adms_check_list_question_id, clq.id clq_id, clq.question_description, clq.points, clq.weight_question,
                cls.adms_sits_question_id, clsq.name_sit
    ...
    public function listAdd() {
        ...
        $list->fullRead("SELECT COUNT(id) resp_result FROM adms_check_list_stores WHERE hash_id =:hash AND adms_sits_question_id <>:sit_id", "hash={$this->DadosId}&sit_id=1");
        $register['countHashResp'] = $list->getResult();

        $list->fullRead("SELECT COUNT(id) no_resp_result FROM adms_check_list_stores WHERE hash_id =:hash AND adms_sits_question_id =:sit_id", "hash={$this->DadosId}&sit_id=1");
        $register['countHashNoResp'] = $list->getResult();

        $list->fullRead("SELECT id cla_id, name name_area FROM adms_check_list_areas WHERE adms_sit_id =:sit", "sit=1");
