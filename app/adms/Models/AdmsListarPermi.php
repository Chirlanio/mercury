...
/**
 * Description of AdmsListarPermi
 *
...
class AdmsListarPermi {
    ...
    public function listarPermi($PageId = null, $NivId = null) {
        ...
        $paginacao->paginacao("SELECT COUNT(nivpg.id) AS num_result FROM adms_nivacs_pgs nivpg INNER JOIN adms_paginas pg ON pg.id=nivpg.adms_pagina_id INNER JOIN adms_niveis_acessos nivac ON nivac.id=nivpg.adms_niveis_acesso_id WHERE nivpg.adms_niveis_acesso_id =:adms_niveis_acesso_id AND nivac.ordem >=:ordem AND (((SELECT permissao FROM adms_nivacs_pgs WHERE adms_pagina_id=nivpg.adms_pagina_id AND adms_niveis_acesso_id={$_SESSION['adms_niveis_acesso_id']}) = 1) OR (pg.lib_pub = 1))", "adms_niveis_acesso_id={$this->NivId}&ordem=" . $_SESSION['ordem_nivac']);
