...
class AdmsEditarTransf {
    ...
    public function verTransf($DadosId) {
        ...
        if (($_SESSION['adms_niveis_acesso_id'] == 4) OR ($_SESSION['adms_niveis_acesso_id'] == 5)) {
            $verTransf->fullRead("SELECT t.id, t.loja_origem_id, t.loja_destino_id, t.nf, t.qtd_vol, t.qtd_prod, t.tipo_transf_id, t.status_id, t.recebido, l.nome loja_ori, ld.nome loja_dest, tt.id tipo_id, st.id sit_id FROM tb_transferencias t INNER JOIN tb_lojas l ON l.id=t.loja_origem_id INNER JOIN tb_lojas ld ON ld.id=t.loja_destino_id INNER JOIN tb_tipo_transf tt ON tt.id=t.tipo_transf_id INNER JOIN tb_status_transf st ON st.id=t.status_id WHERE t.id =:id AND t.status_id <:status_id LIMIT :limit", "id=" . $this->DadosId . "&status_id=2&limit=1");
        } else {
            $verTransf->fullRead("SELECT t.id, t.loja_origem_id, t.loja_destino_id, t.nf, t.qtd_vol, t.qtd_prod, t.tipo_transf_id, t.status_id, t.recebido, l.nome loja_ori, tt.id tipo_id, st.id sit_id FROM tb_transferencias t INNER JOIN tb_lojas l ON l.id=t.loja_origem_id INNER JOIN tb_tipo_transf tt ON tt.id=t.tipo_transf_id INNER JOIN tb_status_transf st ON st.id=t.status_id WHERE t.id =:id LIMIT :limit", "id=" . $this->DadosId . "&limit=1");
        }
