...
class AdmsViewOrderPayment {
    ...
    public function viewOrder($DadosId) {
        ...
        $viewOrder->fullRead("SELECT o.*, a.name area, cc.name costCenter, bs.brand, sp.fantasy_name, tp.name typePayment, bk.bank_name bank,
                tk.name type_key, mn.name manager, so.exibition_name, user.nome user_name_cad, usrUp.nome user_name_up
                ...
                LEFT JOIN adms_usuarios user ON user.id = o.adms_user_id
                LEFT JOIN adms_usuarios usrUp ON usrUp.id = o.update_user_id
                LEFT JOIN adms_banks bk ON bk.id = o.bank_id LEFT JOIN adms_type_key_pixs tk ON tk.id = o.adms_type_key_pix_id LEFT JOIN adms_managers mn ON mn.id = o.manager_id LEFT JOIN adms_sits_order_payments so ON so.id = o.adms_sits_order_pay_id WHERE o.id =:id LIMIT :limit", "id=" . $this->DadosId . "&limit=1");
    ...
    public function listInstallments($InstallmentId) {
        ...
        $installment->fullRead("SELECT id inst_id, adms_order_payment_id, installment, installment_value, date_payment
                FROM adms_installments WHERE adms_order_payment_id =:adms_order_payment_id", "adms_order_payment_id=" . $this->InstallmentId);
