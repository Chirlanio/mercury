...
class AdmsListarPed {
    ...
    public function listarPed($PageId = null) {
        ...
        if ($_SESSION['ordem_nivac'] <= FINANCIALPERMITION) {
            $listarCor->fullRead("SELECT p.*, t.nome tam, l.nome loja, f.nome func, tr.nome sit, c.nome n_cor, c.cor FROM tb_prateleira_infinita p INNER JOIN tb_tam t ON t.id=p.tam_id INNER JOIN tb_lojas l ON l.id=p.loja_id INNER JOIN tb_funcionarios f ON f.id=p.func_id INNER JOIN tb_status_prat tr ON tr.id=p.status_id INNER JOIN adms_cors c ON c.id=tr.cor_id ORDER BY id ASC LIMIT :limit OFFSET :offset", "&limit={$this->LimiteResultado}&offset={$paginacao->getOffset()}");
        } else {
            $listarCor->fullRead("SELECT p.*, t.nome tam, l.nome loja, f.nome func, tr.nome sit, c.nome n_cor, c.cor FROM tb_prateleira_infinita p INNER JOIN tb_tam t ON t.id=p.tam_id INNER JOIN tb_lojas l ON l.id=p.loja_id INNER JOIN tb_funcionarios f ON f.id=p.func_id INNER JOIN tb_status_prat tr ON tr.id=p.status_id INNER JOIN adms_cors c ON c.id=tr.cor_id WHERE p.loja_id =:loja_id ORDER BY id ASC LIMIT :limit OFFSET :offset", "loja_id=" . $_SESSION['usuario_loja'] . "&limit={$this->LimiteResultado}&offset={$paginacao->getOffset()}");
        }
