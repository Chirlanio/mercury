...
/**
 * Description of AdmsValUsuarioTreinamento
 *
 * @copyright (c) year, Chirlanio Silva - Grupo Meia Sola
 */
class AdmsValUsuarioTreinamento {
    ...
    public function valUsuario($Usuario, $EditarUnico = null, $DadoId = null) {
        ...
        if (!empty($this->EditarUnico) AND ($this->EditarUnico == true)) {
            $valUsuario->fullRead("SELECT id FROM adms_users_treinamentos WHERE usuario =:usuario AND id <>:id LIMIT :limit", "usuario={$this->Usuario}&limit=1&id={$this->DadoId}");
        } else {
