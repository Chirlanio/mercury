...
class AdmsVerOrdemServico {
    ...
    public function verOrdemServico($DadosId) {
        ...
        $verOrdemServico->fullRead("SELECT os.*, lj.nome loja,
                se.nome sit, tp.nome tipo, t.nome tam, m.nome marca, ljc.nome loja_conserto, dl.descricao defeito, do.descricao detalhe, ld.descricao local
                ...
                INNER JOIN adms_defeitos_ordem_servico dl ON dl.id=os.def_id
                INNER JOIN adms_detalhes_ordem_servico do ON do.id=os.det_id
                INNER JOIN adms_def_local_ordem_servico ld ON ld.id=os.loc_id
